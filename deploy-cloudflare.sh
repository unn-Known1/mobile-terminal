...
if [ ! -f "$CLOUDFLARED_PATH" ]; then
    ...
    curl -sL "$CLOUDFLARE_URL" -o "$CLOUDFLARED_PATH"
...
$CLOUDFLARED_PATH tunnel --url "http://localhost:$PORT" 2>&1 &
TUNNEL_PID=$!
...
for i in {1..30}; do
    OUTPUT=$(ps aux 2>/dev/null | grep -v grep | grep cloudflared || true)
